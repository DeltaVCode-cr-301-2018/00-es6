...
function displayPics(){
  ...
  // let declares variable that are limited while var declares variable that are global.
  console.log(viewed)
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      let song = `${event.target.id} has ${allProducts[i]} votes in ${allProducts[i]} views`;
      console.log(song);
    }
  }
...
function showList() {
  for(const i = 0; i < allProducts.length; i++) {
    ...
    liEl.textContent = `${allProducts[i]} has ${allProducts[i]} votes in ${allProducts[i]} views`;
